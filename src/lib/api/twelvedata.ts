...
function apiKey(): string {
  return process.env.NEXT_PUBLIC_TWELVEDATA_API_KEY?.trim() || "demo";
}
...
function twelveDataSymbolQueryParams(symbol: string, exchange?: string): string {
  ...
  const q = new URLSearchParams({ symbol: sym, apikey: apiKey() });
...
/** Symbol / company name search for equities (TwelveData `symbol_search`). */
export async function twelveDataSymbolSearch(
  ...
  const q = query.trim();
  if (!q) return [];
  const url = `${BASE}/symbol_search?symbol=${encodeURIComponent(q)}&apikey=${encodeURIComponent(apiKey())}`;
  const res = await fetch(url, { signal });
  if (!res.ok) return [];
